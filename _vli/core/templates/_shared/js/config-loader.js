/* ===========================================================
   VLI CONFIG LOADER - compartilhado entre todos os templates
   Le ?lp=<id> da URL, busca no localStorage e aplica.
   Paletas vem de _shared/data/palettes.json (1 fonte da verdade).
...
(function () {
  ...
  if (!raw) {
    console.warn('[vli] Sem dados no localStorage');
    return;
  }
  ...
  if (!lp) {
    console.warn('[vli] LP nao encontrada:', PARAM);
    return;
  }

  /* ---------- 1. PALETA (injecao imediata, vai no <head>) ---------- */
  // Carrega palettes.json inline (gerado no build) ou usa fallback embarcado
  const PALETTES = window.__VLI_PALETTES__ || {
  ...
  /* ---------- 2. DADOS (depois do DOM pronto) ---------- */
  document.addEventListener('DOMContentLoaded', function applyData() {
    const SECTION_SELECTORS = {
      resumo: '#resumo', galeria: '#galeria', diferenciais: '#diferenciais',
      plantas: '#plantas', ficha: '#ficha', localizacao: '#localizacao',
      faq: '#faq', comercial: '#preco', bairro: '#bairro'
    };
    ...
    const stat = (key, value, suffix = '') => {
      if (value === '' || value == null) return;
      document.querySelectorAll(`[data-stat="${key}"]`).forEach((el) => { el.textContent = `${value}${suffix}`; });
    };
    ...
    const buildFaqSchema = () => {
      ...
      const items = [];
      if (lp.dorms) items.push({ '@type': 'Question', name: `Quantos dormitorios tem ${lp.nome || 'o imovel'}?`, acceptedAnswer: { '@type': 'Answer', text: `${lp.nome || 'O imovel'} possui ${lp.dorms} dormitorios${lp.suites ? `, sendo ${lp.suites} suites.` : '.'}` } });
      if (lp.m2min || lp.m2max) { const area = lp.m2min && lp.m2max ? `${lp.m2min}m2 a ${lp.m2max}m2` : `${lp.m2max || lp.m2min}m2`; items.push({ '@type': 'Question', name: 'Qual a metragem do imovel?', acceptedAnswer: { '@type': 'Answer', text: `A metragem informada para o imovel e ${area}.` } }); }
      if (lp.bairro || lp.cidade) items.push({ '@type': 'Question', name: `Onde fica ${lp.nome || 'o imovel'}?`, acceptedAnswer: { '@type': 'Answer', text: `${lp.nome || 'O imovel'} esta localizado em ${[lp.bairro, lp.cidade].filter(Boolean).join(', ')}${lp.uf ? ` - ${lp.uf}` : ''}.` } });
      if (lp.vagas) items.push({ '@type': 'Question', name: 'Quantas vagas de garagem o imovel possui?', acceptedAnswer: { '@type': 'Answer', text: `O imovel possui ${lp.vagas} vagas de garagem.` } });
      if (lp.status) items.push({ '@type': 'Question', name: 'Qual o status do imovel?', acceptedAnswer: { '@type': 'Answer', text: `O status informado para o imovel e ${lp.status}.` } });
      if (Array.isArray(lp.diferenciais) && lp.diferenciais.length) { const labels = lp.diferenciais.map((i) => i.label).filter(Boolean); if (labels.length) items.push({ '@type': 'Question', name: 'Quais sao os diferenciais do imovel?', acceptedAnswer: { '@type': 'Answer', text: `Entre os principais diferenciais estao: ${labels.join(', ')}.` } }); }
    ...
    if (lp.preco) set('[data-bind="preco"]', lp.preco);
    else document.querySelectorAll('[data-price-block]').forEach((el) => { el.style.display = 'none'; });
    ...
    if (lp.wpp) {
      ...
      document.querySelectorAll('[data-wpp-link]').forEach((link) => { link.href = `https://wa.me/${lp.wpp}?text=${msg}`; });
    }
    ...
    updateJsonLd('schema-organization', { '@context': 'https://schema.org', '@type': 'Organization', name: orgName || lp.nome || 'Imobiliaria', ...(lp.tel ? { telephone: lp.tel } : {}) });
    updateJsonLd('schema-faq', buildFaqSchema());
    updateJsonLd('schema-product', { '@context': 'https://schema.org', '@type': 'Product', name: `${lp.nome || 'Imovel'}${lp.tipo ? ` - ${lp.tipo}` : ''}`, description: metaDescription, ...(heroImage ? { image: heroImage } : {}), ...(orgName ? { brand: { '@type': 'Organization', name: orgName } } : {}), ...(priceToNumber(lp.preco) ? { offers: { '@type': 'Offer', priceCurrency: 'BRL', price: priceToNumber(lp.preco) } } : {}) });

    console.info('[vli] LP carregada:', lp.nome || lp.slug, '- paleta:', lp.palette);
  });
})();
